    ...
    ngOnInit() {
        this.route.paramMap.subscribe(paramMap => {
            if (paramMap.get('category') === 'starred') {
                this.mails$ = this.inboxService.getStarred().pipe(map(mails => mails.sort((a: any, b: any) => b.when - a.when)));
            } else {
                this.mails$ = this.inboxService
                    .getGroup(paramMap.get('category'))
                    .pipe(map(mails => mails.sort((a: any, b: any) => b.when - a.when)));
            }
        });
    }
