...
export class InboxService {
    ...
    getStarred() {
        return of(this.mails).pipe(map(mails => mails.filter(mail => mail.starred)));
    }

    getGroup(group: string) {
        return of(this.mails).pipe(map(mails => mails.filter(mail => mail.group === group)));
    }

    getMail(id: number | string) {
        return of(this.mails).pipe(map(mails => mails.find(mail => mail.id === id)));
    }
